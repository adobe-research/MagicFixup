...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
                ...
                self.max_images > 0) or (self.validate_next < 5 and split != "train"):
            logger = type(pl_module.logger)
            if split != "train":
                self.validate_next += 1
...
if __name__ == "__main__":
    ...
    def get_model(config_path, ckpt_path):
        ...
        model.load_state_dict(torch.load('pretrained_models/sd-v1-4-modified-9channel.ckpt',map_location='cpu')['state_dict'],strict=False)
        ...
        return model
    config_path = 'configs/collage_flow_train.yaml'

    # model = get_model('configs/collage_mix_train.yaml', '/sensei-fs/users/halzayer/collage2photo/Paint-by-Example/official_checkpoint_image_attn_80k.pt')
    ...
    if not opt.resume:
        if opt.train_from_scratch:
            ...
            sd = torch.load(opt.pretrained_model,map_location='cpu')['state_dict']
            
            new_dict = dict()
            for key in sd:
                if 'attn1' in key:
                    new_dict[key.replace('attn1', 'attn3')] = sd[key]
            for key in sd:
                if 'diffusion_model' in key:
                    new_dict[key.replace('diffusion_model', 'reference_model')] = sd[key]
            for new_key in new_dict:
                sd[new_key] = new_dict[new_key]
    ...
    if hasattr(model, "monitor"):
        ...
        default_modelckpt_cfg["params"]["save_weights_only"] = True
    ...
    # TODO change val batches again
    # trainer = Trainer(num_nodes=1, num_sanity_val_steps=1, max_epochs=300, accelerator='gpu', devices=1,
    #                   limit_val_batches=1.0, limit_test_batches=1.0, **trainer_kwargs)
    # trainer = Trainer(num_nodes=1, num_sanity_val_steps=1, max_epochs=300, accelerator='gpu', strategy='ddp_find_unused_parameters_true', devices=8,
    #                   limit_val_batches=1.0, limit_test_batches=1.0, **trainer_kwargs)
    
    # this is the right one
    trainer = Trainer(num_nodes=1, num_sanity_val_steps=1, max_epochs=8, accelerator='gpu', strategy="deepspeed_stage_1", precision="32", devices=8,
                    limit_val_batches=1.0, limit_test_batches=1.0, **trainer_kwargs)
    # trainer = Trainer(num_nodes=1, num_sanity_val_steps=1, max_epochs=10000, accelerator='gpu', strategy="deepspeed_stage_1", precision="32", devices=8,
    #                 overfit_batches=20,limit_val_batches=0.5, limit_test_batches=0.5, check_val_every_n_epoch=1000, **trainer_kwargs)
